    ...
    with get_db() as conn:
        ...
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                pluggy_client_id TEXT DEFAULT '',
                pluggy_client_secret TEXT DEFAULT '',
                pluggy_items TEXT DEFAULT '[]',
        ...
        existing_columns = {
            ...
            for t in ("accounts", "investments", "sync_log")
        }
        ...
        for table, col in mig_cols.items():
            if col not in existing_columns.get(table, set()):
                conn.execute(f"ALTER TABLE {table} ADD COLUMN {col} INTEGER NOT NULL DEFAULT 1")

        # ── Migration: add pluggy columns to users ────────────
        user_cols = set(
            row[1]
            for row in conn.execute("PRAGMA table_info(users)").fetchall()
        )
        for col in ("pluggy_client_id", "pluggy_client_secret", "pluggy_items"):
        ...
        if not admin:
            # Import bcrypt here to avoid circular import at module level
            import bcrypt
            hashed = bcrypt.hashpw(b"admin123", bcrypt.gensalt()).decode("utf-8")
            conn.execute(
                "INSERT INTO users (username, password_hash, display_name, "
                "pluggy_client_id, pluggy_client_secret, pluggy_items) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    "admin",
                    hashed,
                    "Admin",
                    os.getenv("PLUGGY_CLIENT_ID", ""),
                    os.getenv("PLUGGY_CLIENT_SECRET", ""),
                    "[]",
                ),
            )
...
def get_user(conn: sqlite3.Connection, user_id: int) -> Optional[dict]:
    """Get a user by id."""
    row = conn.execute(
        "SELECT id, username, display_name, pluggy_client_id, "
        "pluggy_client_secret, pluggy_items, created_at, updated_at "
        "FROM users WHERE id = ?",
        (user_id,),
    ).fetchone()
...
def get_user_with_password_hash(conn: sqlite3.Connection, username: str) -> Optional[dict]:
    """Get a user by username INCLUDING the password_hash (auth only)."""
    row = conn.execute(
...
def upsert_account(conn: sqlite3.Connection, account: dict, user_id: int = 1) -> str:
    ...
    conn.execute(
        ...
        INSERT INTO accounts (id, name, type, subtype, balance, currency, item_id,
                              ...
                              overdraft_limit, raw_data, updated_at, user_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
...
def upsert_investment(conn: sqlite3.Connection, inv: dict, item_id: str, user_id: int = 1) -> str:
    ...
    conn.execute(
        ...
        INSERT INTO investments (id, item_id, name, type, subtype, balance, amount,
                                 ...
                                 date, status, raw_data, updated_at, user_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
        ON CONFLICT(id) DO UPDATE SET
            item_id = excluded.item_id,
            name = excluded.name,
...
def log_sync(
    ...
    cursor = conn.execute(
        ...
        INSERT INTO sync_log (status, items_count, accounts_count,
                              transactions_count, investments_count, error_message, user_id)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (status, items_count, accounts_count, transactions_count, investments_count, error_message, user_id),
    )
    return cursor.lastrowid


def get_last_sync(user_id: int = 1) -> Optional[dict]:
    """Get the most recent sync log entry for a user."""
    with get_db() as conn:
        row = conn.execute(
